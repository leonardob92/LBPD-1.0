function [] = waveform_plotting_local_v3(S)
...
%                    -S.lineplot:       first number (positive) indicating where the lines showing significant time-windows should be placed (FIRST NUMBER NOT USEFUL ANYMORE).
...
%                    -S.sbrim:          percentage (e.g. 0.15) used to scale the lines or the markers showing the significance 
...
%                                       Do not provide the field if you want to have line colors instead of black markers.
...
% else
    ...
    if isfield(S,'y_lim')
        if ~isempty(S.y_lim)
            ...
            ylims = get(gca,'YLim');
            YLIM(1) = ylims(1) - sum(abs(ylims))*0.1;
            YLIM(2) = ylims(2) + sum(abs(ylims))*0.1;
        end
    else
        ylims = get(gca,'YLim');
        YLIM(1) = ylims(1) - sum(abs(ylims))*0.1;
                ...
                if ~isempty(S.signtp_col) %if the field is not empty
                    if isempty(linep) %if you do not want the significant time-windows plotted as lines
                        ...
                        for ff = round(find(sgf2(1)==time)):round(find(sgf2(2)==time))
                            ...
                            %%% NEW SOLUTION - LEONARDO, OXFORD, 11/07/2024 %%%
                            %%% IF BETTER, DO THE SAME FOR waveplot_groups_local_v2.mat %%%
                            if linep(2) == 1
                                sbrim = S.sbrim; % = 0.15; %to make the markers more far away from the y limits of the figure
                                %working on space between automatic limit of the figure and limit provided by the user  
                                YY = linspace(YLIM(1)+((abs(YLIM(1))+abs(ylims(linep(2))))*sbrim),ylims(linep(2))+((abs(YLIM(1))+abs(ylims(linep(2))))*sbrim),length(S.colorline)+1);
%                                 YY = linspace(YLIM(1)+abs(YLIM(1)*sbrim),ylims(linep(2))+abs(ylims(linep(2))*sbrim),length(S.colorsign)+1);
                                if isfield(S,'colorsign')
                                    if ~iscell(S.colorsign) %if you provide colors
                                        plot([sgf2(1),sgf2(end)],[YY(end-S.signtp_col(iii)),YY(end-S.signtp_col(iii))],'color',S.colorsign(S.signtp_col(iii),:),'LineStyle','-','HandleVisibility','off','LineWidth',1.5)
                                    else %you provide markers
                                        plot([sgf2(1),sgf2(end)],[YY(end-S.signtp_col(iii)),YY(end-S.signtp_col(iii))],'color',S.colorline(S.signtp_col(iii),:),'LineStyle','-','Marker',S.colorsign{S.signtp_col(iii)},'MarkerSize',5,'HandleVisibility','off','LineWidth',1.5)
                                    end
                                else
                                    plot([sgf2(1),sgf2(end)],[YY(end-S.signtp_col(iii)),YY(end-S.signtp_col(iii))],'color',COL(S.signtp_col(iii),:),'LineStyle','-','HandleVisibility','off','LineWidth',1.5)
                                end
                            else %otherwise, for obtaining the same results, I have to add
                                sbrim = S.sbrim; %= 0.15; %to make the markers more far away from the y limits of the figure
                                %working on space between automatic limit of the figure and limit provided by the user  
                                YY = linspace(YLIM(2)-((abs(YLIM(2))+abs(ylims(linep(2))))*sbrim),ylims(linep(2))+((abs(YLIM(2))+abs(ylims(linep(2))))*sbrim),length(S.colorline)+1);
%                                 YY = linspace(YLIM(1)+abs(YLIM(1)*sbrim),ylims(linep(2))+abs(ylims(linep(2))*sbrim),length(S.colorsign)+1);
                                if isfield(S,'colorsign')
                                    if ~iscell(S.colorsign) %if you provide colors
                                        plot([sgf2(1),sgf2(end)],[YY(end-S.signtp_col(iii)),YY(end+S.signtp_col(iii))],'color',S.colorsign(S.signtp_col(iii),:),'LineStyle','-','HandleVisibility','off','LineWidth',1.5)
