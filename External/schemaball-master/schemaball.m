...
%leonardo changed for working around the problem of the 'nan' for plotting the nodes that are calculated considering the averaged connections of the single node with the other ones
rdum = zeros(sz(1),sz(2));
for hh = 1:sz(1)
    for gg = 1:sz(2)
        if ~isnan(r(hh,gg))
            rdum(hh,gg) = r(hh,gg);
...
[Z,isrt]   = sort(accumarray(subs,abs(rdum( row + (col-1)*sz(1) )),[],@mean)); %original with 'r'.. leonardo
...
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.1, y( ipos)*1.1, lbls( ipos),'Color',tcolor,'fontsize',6); %leonardo added the 'fontsize' setting
