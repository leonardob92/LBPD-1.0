...
%       set all transparent pixels in frame to transparencyidx=256
...
function writeaframe()
	if tfhastc
		if f==1;
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'Loopcount',inf,'TransparentColor',tcidxthisframe,'DisposalMethod',disposalmethod{f});
		else
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'WriteMode','append','TransparentColor',tcidxthisframe,'DisposalMethod',disposalmethod{f});
		end
	else
		if f==1;
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'Loopcount',inf,'DisposalMethod',disposalmethod{f});
		else
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'WriteMode','append','DisposalMethod',disposalmethod{f});
		end
