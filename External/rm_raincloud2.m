% Use like: h = rm_raincloud2(data, colours, plot_top_to_bottom, density_type, bandwidth)
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        ...
        % plot patches
        %previous
%         h.p{i, j} = patch('Faces', faces{i, j}, 'Vertices', verts{i, j}, 'FaceVertexCData', colours(j, :), 'FaceColor', 'flat', 'EdgeColor', 'none', 'FaceAlpha', 0.5, 'LineWidth', 0.2);
        %dudu
        h.p{i, j} = patch('Faces', faces{i, j}, 'Vertices', verts{i, j}, 'FaceColor', colours(i, :), 'EdgeColor', 'none', 'LineWidth', 0.2);
...
%         h.s{i, j} = scatter(data{i, j}, -jit{i, j} + ks_offsets(i), 'MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.5, 'SizeData', raindrop_size);
        %dudu
        h.s{i, j} = scatter(data{i, j}, -jit{i, j} + ks_offsets(i), 'MarkerFaceColor', colours(i, :), 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.5, 'SizeData', raindrop_size);
        h.s{i, j}.SizeData = 10;
        %previous
%         h.s{i, j}.MarkerFaceColor = cb(1,:);
%         h.s{i, j}.MarkerEdgeColor = 'red';   
        %dudu
        h.s{i, j}.MarkerFaceColor = cb(1,:);
        h.s{i, j}.MarkerEdgeColor = colours(i, :);   
    end
end
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        ...
        %previous
%         h.m(i, j) = scatter(cell_means(i, j), ks_offsets(i), 'MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', [0 0 0], 'MarkerFaceAlpha', 1, 'SizeData', 40, 'LineWidth', 1);
        %dudu
        h.m(i, j) = scatter(cell_means(i, j), ks_offsets(i), 'MarkerFaceColor', colours(i, :), 'MarkerEdgeColor', [0 0 0], 'MarkerFaceAlpha', 1, 'SizeData', 60, 'LineWidth', 1);
