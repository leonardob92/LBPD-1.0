function O = waveplot_groups_local(S)
O = []; 
...
% THE DIFFERENCE IS THAT AVERAGING (AND STD) IS DONE USING 'MEAN' AND NOT 'NANMEAN'.
...
for cc = 1:length(condition) %over conditions
    for ii = 1:length(roin) %over ROIs
        ...
        for gg = 1:length(glab)
            cnt = cnt + 1;
            %mean
            amean = squeeze(mean(data(roin(ii),:,gsubj{gg},condition(cc)),3));
            %standard error (positive)
            stdey = squeeze(std(data(roin(ii),:,gsubj{gg},condition(cc)),0,3)./sqrt(length(gsubj{gg})));
            if length(condition) == 1
                plot(time,amean,'Color',COL(cnt,:),'LineWidth',1.5,'DisplayName',[glab{gg} ' ' ROIN{ii}])
            else
                plot(time,amean,'Color',COL(cnt,:),'LineWidth',1.5,'DisplayName',[glab{gg} ' ' ROIN{ii} ' ' S.conds{condition(cc)}])
            end
            hold on
            if STE == 1
                plot(time,amean + stdey,':','Color',COL(cnt,:),'LineWidth',1,'HandleVisibility','off')
                hold on
                %standard error (negative)
                plot(time,amean - stdey,':','Color',COL(cnt,:),'LineWidth',1,'HandleVisibility','off')
                hold on
            elseif STE == 2
                fill([time fliplr(time)],[amean + stdey fliplr(amean - stdey)],COL(cnt,:), 'FaceAlpha', transp,'linestyle','none');
                hold on
            end
        end
    end
...
for cc = 1:length(condition) %over conditions
    for ii = 1:length(roin) %over ROIs
        for gg = 1:length(glab)
            ...
            stdey = squeeze(std(data(roin(ii),:,gsubj{gg},condition(cc)),0,3)./sqrt(length(gsubj{gg})));
            if length(condition) == 1
                plot(time,amean,'Color',COL(cnt2,:),'LineWidth',1.5,'DisplayName',[glab{gg} ' ' ROIN{ii}])
            else
                plot(time,amean,'Color',COL(cnt2,:),'LineWidth',1.5,'DisplayName',[glab{gg} ' ' ROIN{ii} ' ' S.conds{condition(cc)}])
            end
            hold on
            if STE == 1
                plot(time,amean + stdey,':','Color',COL(cnt2,:),'LineWidth',1,'HandleVisibility','off')
                hold on
                %standard error (negative)
                plot(time,amean - stdey,':','Color',COL(cnt2,:),'LineWidth',1,'HandleVisibility','off')
                hold on
            elseif STE == 2
                fill([time fliplr(time)],[amean + stdey fliplr(amean - stdey)],COL(cnt2,:), 'FaceAlpha', transp,'linestyle','none','HandleVisibility','off');
                hold on
            end
        end
    end
