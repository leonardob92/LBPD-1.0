...
%             sgf2 = S.signtp; %Find the mean value across the significant time points and create the patch around it (before and after)
%             if ~isempty(S.y_lim)
...
% %             else
% %                 ylims = max(abs(squeeze(mean(mean(mean(S.data(S.chans_index,:,:,S.condition_n)),3),4)))); %get y limits from the max absolute value among your data
...
%             countwave = countwave + 1; %THIS IS FOR GRAD-GRAD-GRAD-MAG-MAG-MAG
            ...
            if countwave <= length(S.chanlabels)
                ...
                for aa = 1:length(S.groups)
                    ...
                    plot(S.time_real,squeeze(mean(mean(S.data(countwave,:,S.gsubj{aa},S.condition_n),3),4)),S.colorline{aa},'LineWidth',1,'DisplayName',S.groups{aa}); %average waveform across all subjects and all conditions (mean(var,3)) = mean across third dimension; (mean(var,4))=mean across fourth dimension)
                    grid minor
        ...
        if ~isempty(S.signtp)
            ...
            if ~isempty(S.y_lim)
                ...
                for ab = 1:length(S.groups)
                    caz(1,ab) = max(squeeze(mean(mean(mean(S.data(S.chans_index,:,S.gsubj{ab},S.condition_n),1),3),4)));
                    caz(2,ab) = min(squeeze(mean(mean(mean(S.data(S.chans_index,:,S.gsubj{ab},S.condition_n),1),3),4)));
                end
...
%         countwave = countwave + 2;
        ...
        for aa = 1:length(S.groups)
            % if more than one channel is specified, compute mean across the selected channels
            %Calculating the standard error mean (SEM) for each experimental group
            data_SEM = nanstd(squeeze(mean(mean(S.data(S.chans_index,:,S.gsubj{aa},S.condition_n),4),1)),0,2)/sqrt(length(S.gsubj{aa}));
            amean = squeeze(mean(mean(mean(S.data(S.chans_index,:,S.gsubj{aa},S.condition_n),1),3),4));
%             hold on
            %plot standard errors (shades..)
%             fillOut = fill([S.time_real fliplr(S.time_real)],[amean+data_SEM fliplr(amean-data_SEM)],S.colorline{aa}, 'FaceAlpha', 0.005,'linestyle','none');
            if S.STE == 1
                fill([S.time_real fliplr(S.time_real)],[amean+data_SEM fliplr(amean-data_SEM)],S.colorline{aa}, 'FaceAlpha', 0.005,'linestyle','none');
                %plot actual mean
                hold on;
            elseif S.STE == 2      
                plot(S.time_real,amean + data_SEM',':','Color',S.colorline{aa},'LineWidth',0.5); %upper std error
%                 xlim(x_lim_temp);
%                 if ~isempty(y_lim_ampl)
%                     ylim(y_lim_ampl);
%                 end
                hold on
                plot(S.time_real,amean - data_SEM',':','Color',S.colorline{aa},'LineWidth',0.5); %lower std error
%                 xlim(x_lim_temp);
%                 if ~isempty(y_lim_ampl)
%                     ylim(y_lim_ampl);
%                 end
                ...
                %             lineOut = plot(S.time_real,amean,S.colorline{aa},'linewidth',1.5,'DisplayName',S.groups{aa}); %% change color or linewidth to adjust mean line
                p1(aa) = plot(S.time_real,amean,'Color',S.colorline{aa},'linewidth',1.5); %% change color or linewidth to adjust mean line
            hold on
        end
        if S.legendl == 1
